...
export default function RecipeDetailClient({
  ...
  useEffect(() => {
    (async () => {
      const r = await fetch(`/api/recipes/${recipe.id}/like`, {
        cache: 'no-store',
        credentials: 'include',
      });
      if (!r.ok) return;
  ...
  useEffect(() => {
    (async () => {
      const r = await fetch(`/api/recipes/${recipe.id}/save`, {
        cache: 'no-store',
        credentials: 'include',
      });
      if (!r.ok) return;
  ...
  const handleShare = async () => {
    ...
    const text = recipe?.description
      ? `ลองดูเมนูนี้: ${recipe.description}`
      : 'ลองดูสูตรนี้';
    try {
      if (navigator.share) {
            ...
            <div className="bg-white/95 border border-[#637402]/20 shadow-sm rounded-2xl p-4 w-[320px]">
              ...
              {editing && (
                <div className="mt-3 space-y-3">
                  <div>
                    <label className="text-xs text-[#637402]/70 font-semibold">
                      Name
                    </label>
                    <input
                      value={draftName}
      ...
      <div className="mt-9">
        <div className="relative w-full px-5">
          ...
          <div className="absolute right-5 top-0 flex items-start gap-5">
            <button
              type="button"
              className={`cursor-pointer transition-transform ${saveLoading ? 'opacity-60 scale-95' : 'hover:scale-105'}`}
            ...
            >
              {!saved ? (
                ...
                >
                  <path
                    fill="#637402"
                    d="M16 2H8a3 3 0 0 0-3 3v16a1 1 0 0 0 .5.87a1 1 0 0 0 1 0l5.5-3.18l5.5 3.18a1 1 0 0 0 .5.13a1 1 0 0 0 .5-.13A1 1 0 0 0 19 21V5a3 3 0 0 0-3-3m1 17.27l-4.5-2.6a1 1 0 0 0-1 0L7 19.27V5a1 1 0 0 1 1-1h8a1 1 0 0 1 1 1Z"
                  />
                </svg>
              ) : (
                <svg
