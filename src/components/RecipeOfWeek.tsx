...
function Profile({ slot, recipe }: { slot: 1 | 2 | 3 | 4; recipe: RecipePick }) {
  ...
  return (
    <div className="flex items-center gap-4">
      <div className="w-[34px] h-[34px] rounded-full overflow-hidden flex-shrink-0 flex items-center justify-center bg-[#637402]">
        {avatar ? (
          <Image src={avatar} alt="person" width={34} height={34} className="w-full h-full object-cover" />
        ) : (
          <DefaultAvatarSVG />
        )}
      </div>
...
function Heart({ isLiked, count, onToggle }: { isLiked: boolean; count?: number; onToggle: () => void }) {
  return (
    <div
    ...
    <div className="flex w-full bg-[#FEFEF6] px-4 rounded-xl relative overflow-hidden min-w-0">
      ...
      <div className="p-5 flex-1 min-w-0 flex flex-col justify-center">
        ...
        <div className="flex items-center justify-between mt-5">
          <p className="py-1.5 px-3 border rounded-3xl text-[14px] font-semibold">{recipe.category ?? '-'}</p>
...
function Modal({ open, onClose, title, children }: { open: boolean; onClose: () => void; title: string; children: React.ReactNode }) {
  ...
  return (
    <div className="fixed inset-0 z-[999]">
      <button type="button" aria-label="close overlay" className="absolute inset-0 bg-black/40 cursor-pointer" onClick={onClose} />
      <div className="absolute left-1/2 top-1/2 w-[min(980px,92vw)] -translate-x-1/2 -translate-y-1/2 rounded-2xl bg-white shadow-xl">
        <div className="flex items-center justify-between px-5 py-4 border-b">
          ...
          <button type="button" onClick={onClose} className="rounded-xl border px-3 py-1 text-sm hover:bg-gray-50 cursor-pointer">ปิด</button>
        </div>
...
export default function RecipeOfWeek({ isAdmin = false }: { isAdmin?: boolean }) {
  ...
  useEffect(() => {
    if (!slots) return;
    const ids = [slots[1]?.recipe?.id, slots[2]?.recipe?.id, slots[3]?.recipe?.id, slots[4]?.recipe?.id]
    ...
    return (
      <div className="bg-[#F9F7EB] h-[600px] flex items-center justify-center">
        <div className="flex flex-col items-center gap-4">
          ...
          <p className="text-[#637402] font-semibold">กำลังโหลดข้อมูลเมนูแนะนำ...</p>
        </div>
      </div>
    );
  }
          ...
          <div className="bg-[#FEFEF6] p-5 rounded-2xl flex flex-col">
            ...
            <div className="flex justify-center flex-1">
              <div className="w-full max-w-[520px] flex flex-col justify-center">
                <div className="my-5 w-full overflow-hidden rounded-xl relative" style={{ aspectRatio: '5 / 4' }}>
                  <Image
                ...
                <div className="flex justify-between items-center">
                  <p className="text-[14px] font-semibold py-1 px-7 border rounded-3xl">{left.category ?? 'curry'}</p>
                  <Heart isLiked={!!likes[left.id]} count={likeCounts[left.id]} onToggle={() => handleToggle(left.id)} />
                </div>
              ...
              return (
                ...
                >
                  <div className="relative h-[150px] w-full bg-gray-100">
                    <Image src={r.coverImage ?? '/nodata.png'} alt={r.name ?? 'recipe'} fill className="object-cover transition-transform duration-300 group-hover:scale-[1.03]" />
                    {active && <div className="absolute top-2 right-2 text-[11px] px-2 py-1 rounded-full bg-[#637402] text-white shadow">เลือกแล้ว</div>}
                  </div>
                  <div className="p-3">
                    <div className="font-semibold text-sm leading-snug line-clamp-2">{r.name ?? '(no name)'}</div>
                    <div className="mt-1 text-xs text-gray-500 flex items-center justify-between">
                      <span className="truncate">{r.category ?? '-'}</span>
                      <span className="opacity-0 group-hover:opacity-100 transition text-[#637402]">เลือก →</span>
                    </div>
                  </div>
                </button>
              );
